...
const Board = ({ data, index, getData }) => {
    ...
    return (
        <>
                ...
                <div className="overflow-y-auto scrollbar-thin scrollbar-gray-100 scrollbar-thumb-gray-200">
                    <div className="grid gap-3 max-h-screen">
                        {data?.task.map((items, index) => (
                            <TaskCard key={index} items={items} />
                        ))}
            ...
            >
                <CreateTaskModal boardId={data._id} onClose={() => showCreateTaskModal(false)} getData={getData} />
            </Modal>
            ...
            >
                <DeleteConfirmModal
                    description={"You won't be able to revert this! All data in the board will be deleted"}
